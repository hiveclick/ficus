...
class LoginAction extends BasicAction
{
	...
	public function execute ()
	{
		...
		$administrator->populate($_REQUEST);
		...
		if (isset($_REQUEST['token'])) {
			try {
				$administrator->tryTokenLogin();
				if (\MongoId::isValid($administrator->getId())) {
					// Successful token authentication
					setcookie('__cookie', (string)$administrator->getId(), (time() + 259200), "/", false);
					$this->getContext()->getUser()->setUserDetails($administrator);
		...
		if ($this->getContext()->getRequest()->getMethod() == \Mojavi\Request\Request::GET) {
			...
			if (strlen($administrator->getEmail()) == 0 || strlen($administrator->getPassword()) == 0) {
