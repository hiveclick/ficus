        ...
        <div class="container-fluid">
    		...
    		<div>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    <li role="presentation" class=""><a href="#clockin" aria-controls="clock-in" role="tab" data-toggle="tab">Clock In</a></li>
                    <li role="presentation" class="active"><a href="#clockout" aria-controls="clock-out" role="tab" data-toggle="tab">Clock Out</a></li>
                </ul>
                <br />
                <!-- Tab panes -->
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane fade <?php echo !\MongoId::isValid($shift->getId()) ? 'in active' : '' ?>" id="clockin">
                        <form id="clockin_form" name="clockin_form" method="POST" action="/clock-in">
                            ...
                            <div class="form-group">
                    			...
                    			<select name="client">
                                    <optgroup label="Clients close to me">
                                        <?php 
                                            /* @var $client \Ficus\Client */
                                            foreach ($clients as $client) {
                                        ?>
                                            <?php if ($client->getDistance($geo_array['latitude'], $geo_array['longitude']) < 100) { ?>
                                                <option value="<?php echo $client->getId() ?>" <?php echo $shift->getClient()->getId() == $client->getId() ? 'selected' : '' ?>><?php echo $client->getName(true) ?></option>
                                            <?php } ?>
                                        <?php } ?>
                                    </optgroup>
                                    <optgroup label="All my clients">
                                        <?php 
                                            /* @var $client \Ficus\Client */
                                            foreach ($clients as $client) {
                                        ?>
                    ...
                    <div role="tabpanel" class="tab-pane fade <?php echo \MongoId::isValid($shift->getId()) ? 'in active' : '' ?>" id="clockout">
                        <form id="clockout_form" name="clockout_form" method="POST" action="/clock-out">
                                    ...
                                    <optgroup label="Primary ADLs">
                                        ...
                                        ?>
                                            <?php if ($adl->getIsPrimary()) { ?>
                                                <option value="<?php echo $adl->getId() ?>" <?php echo $shift->isAdlSelected($adl->getId()) ? 'selected' : '' ?>><?php echo $adl->getName() ?></option>
                                            <?php } ?>
                                        <?php } ?>
                                    </optgroup>
                            ...
                            <div class="alert alert-warning">We tried to clock you out but noticed you did not have a clock-in.  Enter what time you arrived and please remember to clock in next time.</div>
                            <div class="form-group">
                    			<div class="form-label" for="notes">Enter Clock-In time</b></div>
                    			<input type="text" name="clock_in_time" value="" class="form-control" />
                    		</div>
                            
                            <div class="form-group">
                    			<div class="form-label" for="notes">Enter any <b>additional notes</b></div>
                    			<textarea name="notes" class="form-control" rows="4" placeholder="Record any additional notes or observations here..."><?php echo $shift->getNotes() ?></textarea>
                    		</div>
