...
try {
    ...
    while (count($argv) > 0) {
        ...
        } else if (trim($arg) != '/home/ficus/init/upgrade.sh') {
    ...
    // Start Output
    if (!$is_silent) {
        echo StringTools::consoleColor('Ficus Upgrade Script', StringTools::CONSOLE_COLOR_GREEN) . "\n";
    ...
    $current_version = '0';
    try {
        $preferences = new \Ficus\Preferences();
        ...
        if (is_null($ret_val)) {
            ...
            $preferences->setControl(\Ficus\Preferences::READ_WRITE_ADMIN);
            $preferences->insert();
        }
    ...
    $revision_count = 0;
    $version_folders = scandir($webapp_dir . '/lib/Ficus/Migrations/');
    foreach ($version_folders as $version_folder) {
        if (strpos($version_folder, '.') === 0) { continue; }
        
        if (file_exists($webapp_dir . '/lib/Ficus/Migrations/' . $version_folder)) {
            if (!$is_silent && $debug) { StringTools::consoleWrite(' - Scanning /lib/Ficus/Migrations/' . $version_folder, null, StringTools::CONSOLE_COLOR_YELLOW, true); }
            ...
            if (($migration_revision > $current_version) || ($migration_revision == $arg_version) || ($migrate_greater_versions && ($migration_revision > $arg_version))) {
                $revision_files = scandir($webapp_dir . '/lib/Ficus/Migrations/' . $version_folder);
                foreach ($revision_files as $revision_file) {
                    if (strpos($revision_file, '.') === 0) { continue; }
                    if (strpos($revision_file, '.php') === false) { continue; }
                    if (!$is_silent && $debug) { StringTools::consoleWrite('   - Adding /lib/Ficus/Migrations/' . $version_folder . '/' . $revision_file, null, StringTools::CONSOLE_COLOR_YELLOW, true); }
                    $revision_array[str_replace('rev', '', $migration_revision)][] = array(
                            'file' => $webapp_dir . '/lib/Ficus/Migrations/' . $version_folder . '/' . $revision_file,
    ...
    if (count($revision_array) == 0) {
        ...
        foreach ($revision_array as $migration_version => $revision_files) {
            foreach ($revision_files as $revision_array) {
                ...
                $class_name = 'Ficus\\Migrations\\' . $migration_revision . '\\' . $class_name;
