...
try {
    ...
    $project_dirs = array(
    	'api' => $base_dir . 'api/',
    	'family' => $base_dir . 'family/',
    	'provider' => $base_dir . 'provider/',
    	'care' => $base_dir . 'care/',
    );
    ...
    foreach ($lib_folders as $lib_name => $lib_folder) {
        if (!$is_silent) { StringTools::consoleWrite(' - lib path', $lib_folder, StringTools::CONSOLE_COLOR_YELLOW); }
        foreach ($project_dirs as $project_dir) {
	        if (file_exists($admin_base_dir . 'webapp/lib/' . $lib_folder) && is_dir($admin_base_dir . 'webapp/lib/' . $lib_folder)) {
	            // Setup lib for api
	            if (!file_exists($project_dir . "webapp/lib/" . $lib_folder) && !is_link($project_dir . "webapp/lib/" . $lib_folder)) {
	                $cmd = 'ln -s ' . $admin_base_dir . '/webapp/lib/' . $lib_folder . ' ' . $project_dir . 'webapp/lib/' . $lib_folder;
	                shell_exec($cmd);
	            } else if (is_link($project_dir . "webapp/lib/" . $lib_folder)) {
	                $cmd = 'rm -f  ' . $project_dir . 'webapp/lib/' . $lib_folder;
	                shell_exec($cmd);
    ...
    if (!$is_silent) { StringTools::consoleWrite(' - lib path', 'Saved', StringTools::CONSOLE_COLOR_GREEN, true); }
    ...
    if (file_exists($init_dir . "/config/databases.ini")) {
        ...
        foreach ($project_dirs as $project_dir) {
        	file_put_contents($project_dir . "webapp/config/databases.ini", $config_contents);
        }
    ...
    if (file_exists($init_dir . "/config/config.php.init")) {
        ...
        foreach ($project_dirs as $project_dir) {
        	file_put_contents($project_dir . "webapp/config.php", $config_contents);
        }
    ...
    foreach ($project_dirs as $project_dir) {
    	file_put_contents($project_dir . "docroot/.htaccess", $htaccess_contents);
    }
    ...
    */
    ...
    if (file_exists($init_dir . "/config/factories.ini")) {
        ...
        foreach ($project_dirs as $project_dir) {
       		file_put_contents($project_dir . "webapp/config/factories.ini", $factory_contents);
       	}
    ...
    if (file_exists($init_dir . "/config/logging.ini")) {
        ...
        foreach ($project_dirs as $project_dir) {
        	file_put_contents($project_dir . "webapp/config/logging.ini", $logging_contents);
        }
    ...
    if (file_exists($init_dir . "/config/settings.ini")) {
        ...
        foreach ($project_dirs as $project_dir) {
        	file_put_contents($project_dir . "webapp/config/settings.ini", $settings_contents);
        }
    ...
    foreach ($project_dirs as $project_dir) {
    	$cache_folders[] = ($project_dir . 'webapp/cache/');
    }
    ...
    foreach ($project_dirs as $key => $project_dir) {
    	$global_folders['Cache (' . $key . ')'] = ($project_dir . 'webapp/cache/');
    	$global_folders['Meta (' . $key . ')'] = ($project_dir . 'webapp/cache/');
    }
