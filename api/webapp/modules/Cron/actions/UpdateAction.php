...
class UpdateAction extends BasicConsoleAction {
	...
	{
		try {
			if (file_exists(\Ficus\Updater::UPDATE_FILE)) {
				unlink(\Ficus\Updater::UPDATE_FILE);
			}
			
			/* @var $updater \Ficus\Updater */
			$updater = new \Ficus\Updater();
			...
			$cmd = 'yum update -q -y ' . \Ficus\Updater::PACKAGE_NAME;
			$cmd_output = shell_exec($cmd);
			\Mojavi\Logging\LoggerManager::error(__METHOD__ . " :: " . $cmd_output);
			...
			\Ficus\Settings::saveSetting('version', ($updater->getInstalledPackage()->getVersion() . '.' . $updater->getInstalledPackage()->getRelease()));
